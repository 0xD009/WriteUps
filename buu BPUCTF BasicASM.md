...
comefrom: buu
tools: []
encryptdecrypt: [guess]
whatfile: [s]
language: [asm]
score: 1
flag: "flag{d0_y0u_know_x86-64_a5m?}"
tags:
- re
---
给的是.s文件，vscode打开发现是纯汇编，而且不可编译
看了一下感觉比较烦躁，懒得再看了
```C
...
还有一个文件output：`662e61257b26301d7972751d6b2c6f355f3a38742d74341d61776d7d7d`
非常明显能看出hexflag的结构来了，奇数位没被改，然后去碰碰运气
`00007FF7A8AC5AFA  xor         eax,42h `
这一行，有一个异或，然后尝试一下，还真对了

```python
from Crypto.Util.number import long_to_bytes

cipher = 0x662e61257b26301d7972751d6b2c6f355f3a38742d74341d61776d7d7d

cipher = long_to_bytes(cipher)

flag = ''
for i in cipher:
    if cipher.index(i) % 2: flag += chr(i ^ 0x42)
    else: flag += chr(i)
print(flag)
